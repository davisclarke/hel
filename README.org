...
- In Emacs the cursor ("point" in Emacs terms) is located *between* two
  characters rather than *on* a character like in Helix or Vim. I decided to
  keep this behavior, instead of emulating original one, as Evil does, because
  the primary object of interaction in Helix approach is a selection, not the
  cursor itself.

- =x= and =X= commands are reworked. They are expand and contract line-wise
  selection down when cursor is at the end of the selection, or up when
...
- Keys that are relevant only when multiple cursors are present will be active
  ...
  keymap). This allows to reuse, for example, =K= for documentation lookup or =,= for
